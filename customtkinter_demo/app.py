...
------------------------------------------------------------------------------
【ファイルの役割】
  ・Python で Windows ネイティブのモダンなデスクトップGUIアプリケーションを構築します。
  ・クラス構造（KinProDesktopApp）で整理され、左側の固定サイドバーと右側の画面遷移を管理します。
  ・プロジェクト管理ダッシュボード、テキスト/コードエディタ、ダーク/ライトモード設定を備えています。
  ・PyInstaller を使用することで、単一の .exe 実行ファイルに変換して配布できます。

【ここを変えるとどう変わるか？】
  ・ctk.set_appearance_mode("Light") にする -> 白を基調とした明るいライトモードになります。
  ・ctk.set_default_color_theme("green") にする -> ボタンやアクセントカラーが緑色になります。
  ・self.geometry("800x600") にする -> ウィンドウの初期表示サイズを変更できます。
  ・self.project_data に要素を追加する -> ダッシュボードの初期リストに新しい行が追加されます。
==============================================================================
"""
